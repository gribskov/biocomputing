...
db = dbh.cursor()
db.execute('DROP TABLE IF EXISTS transcript')
db.execute( 'CREATE TABLE transcript (id TEXT, component INTEGER, gene INTEGER, isoform INTEGER)')
...
while trinity.next():
    ...
    db.execute( 'INSERT INTO transcript VALUES (?,?,?,?)',
                (cluster, component, gene, isoform) )

    # n = getLen(trinity.doc)
    # print('m:', n,  '   ', trinity.doc)

    nn=getPath(trinity.doc)
    if len(nn)<=1: continue

    print('doc:',trinity.doc)
    print(trinity.seq)
    for p in nn:
        print('nn:', p)
    #if nseq > 500: break
...
db.execute( 'SELECT id, component, gene, isoform FROM transcript')
n = 0
for row in db:
    n += 1
    print(  row['id'])
